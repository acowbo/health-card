(function (global, factory) {
  ...
  function PlanCard(props) {
    ...
    // 7天计划
    var weekPlan = Array.isArray(props.weekPlan) ? props.weekPlan : [];
    var dayNames = ['\u5468\u4E00', '\u5468\u4E8C', '\u5468\u4E09', '\u5468\u56DB', '\u5468\u4E94', '\u5468\u516D', '\u5468\u65E5'];
    if (weekPlan.length > 0) {
      items.push(e('div', { key: 'week', style: { padding: '10px 12px', background: t.soft, border: '1px solid ' + t.border, borderRadius: '10px', marginBottom: '8px' } },
        e('div', { style: { display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' } },
          e('span', { style: { fontSize: '18px' } }, '\u{1F4C5}'),
          e('span', { style: { fontSize: '12px', fontWeight: 600, color: '#374151' } }, '7\u5929\u8BE6\u7EC6\u8BA1\u5212')
        ),
        weekPlan.map(function (day, i) {
          var dayLabel = day.day || dayNames[i] || ('Day ' + (i + 1));
          return e('div', { key: i, style: { marginBottom: i < weekPlan.length - 1 ? '10px' : 0, paddingBottom: i < weekPlan.length - 1 ? '10px' : 0, borderBottom: i < weekPlan.length - 1 ? '1px solid rgba(0,0,0,0.05)' : 'none' } },
            e('div', { style: { fontSize: '12px', fontWeight: 700, color: t.accent, marginBottom: '4px' } }, dayLabel),
            day.diet ? e('div', { style: { display: 'flex', gap: '6px', marginBottom: '3px' } },
              e('span', { style: { fontSize: '11px', color: '#F97316', fontWeight: 600, flexShrink: 0 } }, '\u{1F37D}\uFE0F\u996E\u98DF\uFF1A'),
              e('span', { style: { fontSize: '11px', color: '#6B7280', flex: 1 } }, day.diet)
            ) : null,
            day.exercise ? e('div', { style: { display: 'flex', gap: '6px' } },
              e('span', { style: { fontSize: '11px', color: '#10B981', fontWeight: 600, flexShrink: 0 } }, '\u{1F3C3}\u8FD0\u52A8\uFF1A'),
              e('span', { style: { fontSize: '11px', color: '#6B7280', flex: 1 } }, day.exercise)
            ) : null
          );
        })
      ));
  ...
  function HealthSmartCard(props) {
    ...
    var data = props;
    var originalContent = null; // 保留原始 content 字符串备用
    ...
    if (typeof props.content === 'string') {
      ...
      if (raw.charAt(0) === '{') {
        try {
          ...
          var jsonStr = endIdx >= 0 ? raw.slice(0, endIdx + 1) : raw;
          var parsed = JSON.parse(jsonStr);
          if (parsed && typeof parsed === 'object') {
            // 提取 JSON 后面的文字部分作为备用 content
            var trailingText = endIdx >= 0 ? raw.slice(endIdx + 1).trim() : '';
            if (trailingText) originalContent = trailingText;
            data = parsed;
          }
        } catch (err) {
          // 解析失败，当纯文字处理
    ...
    // 没有任何结构化数据（包括全是 null 的 JSON）→ 兜底纯文字卡片
    if (!hasStructuredData(data)) {
      var fallbackContent = originalContent || safe(data.feedback, null) || safe(data.summary, null) || safe(data.content, null);
      return FallbackCard({ content: fallbackContent });
    }
